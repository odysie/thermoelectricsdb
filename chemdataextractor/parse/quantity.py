...
#oddy note: this is where the tagging of raw_value happens
# v4 changed the add_actions from merge to join for value_with_power, value_range, and value_single, in order to use the pollutant masking properly
def value_element(units=None):
    pure_number = R('^(([\+\-–−]?\d+(([\.・,\d])+)?)|(\<nUm\>))$')
    fraction = R('^(([\+\-–−]?\d+/\d+)|(\<nUm\>))$') | (R('^(([\+\-–−]?\d+)|(\<nUm\>))$') + R('^/$') + R('^((\d+)|(\<nUm\>))$')).add_action(merge)
    ...
    value_range = (Optional(R('^[\-–−]$')) + (plusminus_range | joined_range | spaced_range | to_range | between_range | bracket_range | spaced_bracket_range))('raw_value').add_action(join)

    value_single = (Optional(R('^[~∼˜\<\>]$')) + Optional(R('^[\-–−]$')) + number)('raw_value').add_action(join)

    #oddy created value with power
    # oddy v7 added rules to capture records from Springer text files which retain the ^ sign in the Document class (nb ^ needs to be escaped in Regex)
    # Markup languges will remove the ^ signs for powers and break the expression up in tokens
    # while simple text files downloaded from Springer will retain the ^ sign and end up with one token
    negative_power_of_ten = ((W('10') + T('HYPH') + number) | R('^10\^[\-–−]\d+$')).add_action(merge) # HTML / XML 5 × 10^-3 will appear as 5 × 10-3, captured by first part. TXT captured by 2nd part (Regex)
    positive_power_of_ten = R('10\^?\d\d?') # HTML/XML 5 × 10^12 will appear as 5 × 1012, while text as 10^12
    value_with_power = (number + W('×') + (positive_power_of_ten | negative_power_of_ten))('raw_value').add_action(join)

    #oddy added value with power in the OR | bracket
    value = Optional(lbrct).hide() + (value_with_power | value_range | value_single)('raw_value') + Optional(rbrct).hide()
...
#oddy note: what extracts value from raw_value
def extract_value(string):
    ...
    #oddy for × case:
    #print(values)
    if '×' in string: #making sure this only happens for power cases
        #negative case
        if len(values) == 3: #meaning that it is the negative case, e.g. 53 × 10−4, which gives values [53.0, 10.0, 4.0]
            try:
                if values[1] == 10: #making sure that the second value is 10
                    values = [values[0] * (10 ** (-1.0 *values[2]))] #needs to be a list, returned as [0.0053] (from 53 * 10 ^ (-4)) in the example
            except (ValueError, IndexError):
                pass

        #positive case
        elif len(values) == 2: #e.g. 6.2 × 1019 (which is really 6.2× 10^(19)) and gives values [6.2, 1019]
            try:
                part_of_ten = float(str(values[1])[:2]) #transform float to string to grab slices (10 and the power 19 in the example) and then transform back to float
                part_of_power = float(str(values[1])[2:])
                if part_of_ten == 10: #making sure the second value is indeed the ten which is lifted to some power
                    values = [values[0] * (10 ** part_of_power)] #needs to be a list, returned as 6.2e+19 in the example
            except (ValueError, IndexError):
                pass
    #new addition, for negative cases without ×, e.g. 10-4. , up to 10-9. Assume anything else is just a range, e.g. 10-13 = 10 to 13
    #note: the positive case without × is missing. But is 103 = 10^3 OR just 103.0
    elif len(values) == 2:
        try:
            if (values[0] == 10) and (values[1] < 10):
                values = [(10 ** (-1.0 *values[1]))]
        except (ValueError, IndexError):
            pass
