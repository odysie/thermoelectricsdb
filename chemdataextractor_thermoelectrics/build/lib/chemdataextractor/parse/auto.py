...
def construct_unit_element(dimensions):
    ...
    # print(f"Elements regex: {units_regex}") #oddy
    ...
    for element in dimensions.units_dict:
        #print(element.pattern) #oddy: I know the problem creeps in from constituent dimensions, so one of them must be the culprit
        units_regex += '(' + element.pattern + ')|'
    units_regex += r'(\/)'
    ...
    units_regex += '$'
    #print(f"UNITS_REGGEX: {units_regex}") #oddy
    #print(f"UNITS_REGGEX2: {units_regex2}") #oddy
    #units_regex = '^W$' #oddy: explicitely setting BOTH regex to be W for the power case fixes the problem. This probably means that the expressions end up becoming too lenient, for some reason.
    #units_regex2 = '^W$' #oddy: but what on earth causes Wand to be picked up, or even worse, Wat300?! I see some powers above... Just notice the space dammit!
    return (R(pattern=units_regex) + ZeroOrMore(R(pattern=units_regex) | R(pattern=units_regex2))).add_action(merge)
    #oddy: so this is what should match things like W cm-1 K-1 etc. But it also matches 'W and any' altogether, due to a fault in regex, which matches things like 'and' coupled with ZeroOrMore which goes over the spaces
...
def match_dimensions_of(model):
    ...
    def check_match(result):
        try:
            #oddy 3 prints. If const_dims exist, then the problem already manifests before the code arrives here.
            # print(f"result: {[res.text for res in result]}")
            # print(f"result[0].text: {result[0].text}")
            # print(f"model.dimensions: {model.dimensions}")
            extract_units(result[0].text, model.dimensions, strict=True) #oddy for the and rainign problem: if u set a W (for power) here, then it will extract, getting raw_units as Wand, and raw_value corect.
            return True
...
class BaseAutoParser(BaseParser):
    ...
    def interpret(self, result, start, end):
        ...
        # print("result etree:")
        # pprint(etree.tostring(result)) #note to oddy: uncomment this line to see what is passed to the interpret function and added extra empty print
        # print()
        if result is None:
            return
        requirements = True
...
class AutoSentenceParser(BaseAutoParser, BaseSentenceParser):
    ...
    def root(self):
        ...
        if hasattr(self.model, 'dimensions') and not self.model.dimensions:
            ...
            # oddy prints
            # print(f"self.model : {self.model}, self.model.dimensions: {self.model.dimensions}")
            # print(f"construct: {construct_unit_element(self.model.dimensions).with_condition(match_dimensions_of(self.model))}")
            unit_element = Group(
                construct_unit_element(self.model.dimensions).with_condition(match_dimensions_of(self.model))('raw_units')) #oddy: If we remove the condition, then this one tags the wrong thing. For some reason, Wand.
            specifier = self.model.specifier.parse_expression('specifier')
            if self.lenient:
